...
[train, id_train] = vl_colsubset(train, size(psi,2));
[test, id_test] = vl_colsubset(test, size(psi,2));
...
n = numel(imageIds) ;
imageIds =  vl_colsubset(imageIds, 10000) ; % change here!
n = numel(imageIds);
% prepare batches
...
%       if iscell(batchResults{b}.code{j})
%           batchResults{b}.code{j} = batchResults{b}.code{j}{1};
%       end
